  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div
        ref={modalRef}
        tabIndex={-1}
        className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl ring-1 ring-slate-200 overflow-hidden outline-none animate-in fade-in zoom-in-95 duration-200"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-start justify-between p-6 border-b border-slate-100 bg-white">
          <div>
            {node ? (
              <div className="flex items-center gap-3 mb-1">
                ...
                <span className="text-sm text-slate-500">Level {node.level}</span>
              </div>
            ) : null}
          ...
          <button
            onClick={onClose}
            className="rounded-full p-2 text-slate-400 hover:text-slate-500 hover:bg-slate-100 transition-colors"
          ...
          {children ? (
            ...
            <>
              ...
              {node.metadata && Object.keys(node.metadata).length > 0 && (
                <div className="mb-6">
                  <h3 className="text-sm font-medium text-slate-900 mb-3">Additional Details</h3>
